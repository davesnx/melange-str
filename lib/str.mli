...
(** [string_match r s start] tests whether a substring of [s] that starts at
    position [start] matches the regular expression [r]. The first character of
...
(** [search_backward r s last] searches the string [s] for a substring matching
    the regular expression [r]. The search first considers substrings that start
    at position [last] and proceeds towards the beginning of string. Return the
    position of the first character of the matched substring.
    @raise Not_found if no substring matches. *)

val string_partial_match : regexp -> string -> int -> bool
(** Similar to {!Str.string_match}, but also returns true if the argument string
    is a prefix of a string that matches. This includes the case of a true
    complete match. *)

val matched_string : string -> string
(** [matched_string s] returns the substring of [s] that was matched by the last
    call to one of the matching or searching functions. The user must make sure
...
(** [matched_group n s] returns the substring of [s] that was matched by the
    [n]th group [\(...\)] of the regular expression. When [n] is [0], it returns
    the substring matched by the whole regular expression.
...
(** [group_end n] returns the position of the character following the last
    character of substring that was matched by the [n]th group of the regular
    expression.
    @raise Not_found if the [n]th group was not matched.
    @raise Invalid_argument if there are fewer than [n] groups. *)
...
(** [global_replace regexp templ s] returns a string identical to [s], except
    ...
    [templ]. The replacement template [templ] can contain [\1], [\2], etc; these
    sequences will be replaced by the text matched by the corresponding group in
    the regular expression. [\0] stands for the text matched by the whole
    regular expression. *)

val replace_first : regexp -> string -> string -> string
(** Same as {!Str.global_replace}, except that only the first substring matching
...
(** [replace_matched repl s] returns the replacement text [repl] in which [\1],
    ...
    matching or searching function. [s] must be the same string that was passed
...
(** [split r s] splits [s] into substrings, taking as delimiters the substrings
    ...
    [split (regexp "[ \t]+") s] splits [s] into blank-separated words. An
    occurrence of the delimiter at the beginning or at the end of the string is
    ignored. *)
...
(** Same as {!Str.split} but occurrences of the delimiter at the beginning and
    at the end of the string are recognized and returned as empty strings in the
    result. *)
...
(** Same as {!Str.split_delim}, but returns the delimiters as well as the
    ...
    the result list; the latter are tagged [Text]. *)

val bounded_full_split : regexp -> string -> int -> split_result list
(** Same as {!Str.bounded_split_delim}, but returns the delimiters as well as
    the substrings contained between delimiters. *)
